@props([
'allGrados',
'allMaterias',
'allMaestros',
'selectedMaestros',
])

<div x-data x-show="filtersOpen" class="mb-6" @filters-cleared.window="$el.querySelectorAll('input[type=checkbox]').forEach(c => c.checked = false); $el.querySelectorAll('input.flatpickr').forEach(i => i._flatpickr.clear());">
    <div class="bg-sigedra-light-bg p-4 rounded-lg border">
        <div class="grid grid-cols-1 md:grid-cols-2 {{ auth()->user()->hasRole('Maestro') ? 'lg:grid-cols-4' : 'lg:grid-cols-5' }} gap-4">
            <!-- Wrapper for date filters -->
            <div class="grid grid-cols-2 gap-4 md:col-span-2 lg:col-span-2">
                <!-- Filtro por fecha -->
                <div class="mt-2">
                    <x-input-label for="start_date">Fecha de inicio</x-input-label>
                    <div class="relative max-w-sm">
                        <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none">
                            <i class="ph ph-calendar w-4 h-4 text-sigedra-text-medium dark:text-sigedra-text-light"></i>
                        </div>
                        <input id="start_date" type="text" class="custom-datepicker" placeholder="Seleccionar fecha" autocomplete="off" wire:model.defer="startDate">
                    </div>
                </div>
                <div class="mt-2">
                    <x-input-label for="end_date">Fecha de fin</x-input-label>
                    <div class="relative max-w-sm">
                        <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none">
                            <i class="ph ph-calendar w-4 h-4 text-sigedra-text-medium dark:text-sigedra-text-light"></i>
                        </div>
                        <input id="end_date" type="text" class="custom-datepicker" placeholder="Seleccionar fecha" autocomplete="off" wire:model.defer="endDate">
                    </div>
                </div>
            </div>

            <!-- Filtro por Grado -->
            <div x-data="{ open: false }" class="relative mt-2">
                <x-input-label for="grades">Grado</x-input-label>
                <button @click="open = !open" type="button" class="custom-select-button">
                    <span class="block truncate pr-8">Seleccionar grados...</span>
                    <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2.5">
                        <i class="ph ph-caret-down text-lg text-sigedra-text-medium"></i>
                    </span>
                </button>
                <div x-show="open" @click.away="open = false" class="absolute z-10 mt-1 w-full bg-white rounded-lg border border-gray-200" style="display: none;">
                    <ul class="max-h-60 rounded-lg py-1 text-base overflow-auto focus:outline-none sm:text-sm">
                        @foreach($allGrados as $anio => $grados)
                        <li x-data="{ expanded: true }" class="py-1">
                            <h3 @click="expanded = !expanded" class="flex items-center justify-between px-3 py-2 text-sm font-semibold text-sigedra-text-dark cursor-pointer hover:bg-gray-50">
                                <span>Año {{ $anio }}</span>
                                <i class="ph ph-caret-down text-lg transition-transform" :class="{'rotate-180': expanded}"></i>
                            </h3>
                            <ul x-show="expanded" class="pl-4 mt-1 space-y-1">
                                @foreach($grados as $grado)
                                <li>
                                    <label class="flex items-center px-3 py-2 cursor-pointer hover:bg-gray-50 rounded-md">
                                        <input wire:model.defer="selectedGrades" type="checkbox" value="{{ $grado->id }}" class="h-4 w-4 text-sigedra-primary rounded focus:ring-sigedra-primary">
                                        <span class="ml-3 block font-normal truncate text-gray-800">{{ $grado->nombre }}</span>
                                    </label>
                                </li>
                                @endforeach
                            </ul>
                        </li>
                        @endforeach
                    </ul>
                </div>
            </div>

            <!-- Filtro por Materia -->
            <div x-data="{ open: false }" class="relative mt-2">
                <x-input-label for="subjects">Materia</x-input-label>
                <button @click="open = !open" type="button" class="custom-select-button">
                    <span class="block truncate pr-8">Seleccionar materias...</span>
                    <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2.5">
                        <i class="ph ph-caret-down text-lg text-sigedra-text-medium"></i>
                    </span>
                </button>
                <div x-show="open" @click.away="open = false" class="absolute z-10 mt-1 w-full bg-white rounded-lg border border-gray-200" style="display: none;">
                    <ul class="max-h-60 rounded-lg py-1 text-base overflow-auto focus:outline-none sm:text-sm">
                        @foreach($allMaterias as $materia)
                        <li>
                            <label class="flex items-center px-3 py-2 cursor-pointer hover:bg-gray-50">
                                <input wire:model.defer="selectedMaterias" type="checkbox" value="{{ $materia->id }}" class="h-4 w-4 text-sigedra-primary rounded focus:ring-sigedra-primary">
                                <span class="ml-3 block font-normal truncate text-gray-800">{{ $materia->nombre }}</span>
                            </label>
                        </li>
                        @endforeach
                    </ul>
                </div>
            </div>

            @if(!auth()->user()->hasRole('Maestro'))
            <!-- Filtro por Maestro -->
            <div x-data="{ open: false }" class="relative mt-2">
                <x-input-label for="maestro">Maestro</x-input-label>
                <button @click="open = !open" type="button" class="custom-select-button">
                    <span class="block truncate pr-8">Seleccionar maestros...</span>
                    <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2.5">
                        <i class="ph ph-caret-down text-lg text-sigedra-text-medium"></i>
                    </span>
                </button>
                <div x-show="open" @click.away="open = false" class="absolute z-10 mt-1 w-full bg-white rounded-lg border border-gray-200" style="display: none;">
                    <ul class="max-h-60 rounded-lg py-1 text-base overflow-auto focus:outline-none sm:text-sm">
                        @foreach($allMaestros as $maestro)
                        <li>
                            <label class="flex items-center px-3 py-2 cursor-pointer hover:bg-gray-50">
                                <input wire:model.defer="selectedMaestros" type="checkbox" value="{{ $maestro->id }}" class="h-4 w-4 text-sigedra-primary rounded focus:ring-sigedra-primary">
                                <span class="ml-3 block font-normal truncate text-gray-800">{{ $maestro->nombre_completo }}</span>
                            </label>
                        </li>
                        @endforeach
                    </ul>
                </div>
            </div>
            @endif
        </div>
        <div class="flex justify-end gap-3 mt-4">
            <x-secondary-button wire:click="clearFilters">Limpiar Filtros</x-secondary-button>
            <x-primary-button wire:click="applyFilters">Aplicar Filtros</x-primary-button>
        </div>
    </div>
</div>
